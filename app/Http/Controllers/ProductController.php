...
class ProductController extends Controller
{
            ...
            if ($request->hasFile('main_image')) {
                try {
                    ...
                    if ($product->main_image) {
                        $this->imageService->deleteProductImage($product->main_image);
                    }
                    
                    // Store new main image
                    $productData['main_image'] = $this->imageService->storeProductImage(
                        $request->file('main_image'),
                        'main'
                    );
                } catch (\Exception $e) {
                    \Log::error('Error updating main image: ' . $e->getMessage());
                    throw new \Exception('Failed to update main image: ' . $e->getMessage());
                }
            }
                    ...
                    if ($image && $index < 4) {
                        try {
                            ...
                            if ($product->$fieldName) {
                                $this->imageService->deleteProductImage($product->$fieldName);
                            }
                            
                            // Store new image
                            $productData[$fieldName] = $this->imageService->storeProductImage(
                                $image,
                                "additional_{$index}"
                            );
                        } catch (\Exception $e) {
                            \Log::error("Error updating additional image {$index}: " . $e->getMessage());
                            throw new \Exception("Failed to update additional image {$index}: " . $e->getMessage());
                        }
                    }
                }
            }
    ...
    public function showEngine()
    {
        ...
        return view('Customer.products.shop', compact('products', 'categories', 'brands'));
    }
}
